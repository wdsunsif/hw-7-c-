static void Main(string[] args)
    {
        ...
        var debtorsWithSpecificEmailDomains = debtors.Where(d => d.Email.EndsWith("@rhyta.com") || d.Email.EndsWith("@dayrep.com"));
        PrintDebtors("Debtors with specific email domains:", debtorsWithSpecificEmailDomains);

        // 3) показать должников в возрасте от 26 до 36 лет.
        var currentDate = DateTime.Now;
        var debtorsAgedBetween26And36 = debtors.Where(d => (currentDate.Year - d.BirthDay.Year) >= 26 && (currentDate.Year - d.BirthDay.Year) <= 36);
        PrintDebtors("Debtors aged between 26 and 36 years:", debtorsAgedBetween26And36);

        // 4) показать должников задолженность которых не превышает 5000.
        var debtorsWithDebtUnder5000 = debtors.Where(d => d.Debt <= 5000);
        PrintDebtors("Debtors with debt not exceeding 5000:", debtorsWithDebtUnder5000);

        // 5) показать должников у которых полное имя состоит из более 18 символов и номер телефона состоит из 2 и более цифр 7.
        var debtorsWithLongNameAndPhoneWithMultiple7s = debtors.Where(d => d.FullName.Length > 18 && d.Phone.Count(c => c == '7') >= 2);
        PrintDebtors("Debtors with full name length more than 18 characters and phone with 2 or more digit 7:", debtorsWithLongNameAndPhoneWithMultiple7s);

        // 7) показать должников рожденных зимой.
        var winterBirthDebtors = debtors.Where(d => d.BirthDay.Month == 12 || d.BirthDay.Month <= 2);
        PrintDebtors("Debtors born in winter:", winterBirthDebtors);
        ...
        var debtorsWithDebtAboveAverageSorted = debtors.Where(d => d.Debt > averageDebt)
                                                        .OrderBy(d => d.FullName.Split(' ').Last())
                                                        .ThenByDescending(d => d.Debt);
        PrintDebtors("Debtors with debt above average, sorted by last name then by descending debt:", debtorsWithDebtAboveAverageSorted);

        // 9) показать фамилии, возраст и общей суммы долга должников, где в номере телефона нет цифры 8
        var debtorsWithNo8InPhoneNumber = debtors.Where(d => !d.Phone.Contains('8'));
        Console.WriteLine("Debtors with no digit 8 in phone number:");
        foreach (var debtor in debtorsWithNo8InPhoneNumber)
        {
            Console.WriteLine($"{debtor.FullName.Split(' ').Last()}, Age: {currentDate.Year - debtor.BirthDay.Year}, Debt: {debtor.Debt}");
        }
        Console.WriteLine();

        // 11) Составьте список должников, у которых в имени и фамилии есть не менее 3 одинаковых букв, и отсортируйте их по алфавиту.
        var debtorsWithAtLeast3SameLetters = debtors.Where(d => d.FullName.ToLower().GroupBy(c => c).Any(g => g.Count() >= 3))
                                                     .OrderBy(d => d.FullName);
        PrintDebtors("Debtors with at least 3 same letters in name and surname, sorted alphabetically:", debtorsWithAtLeast3SameLetters);

        // 13) показать год, в котором родилось наибольшее количество должников
        var mostCommonBirthYear = debtors.GroupBy(d => d.BirthDay.Year)
        ...
        var debtorsWithLargestDebts = debtors.OrderByDescending(d => d.Debt).Take(5);
        PrintDebtors("Top 5 debtors with the largest debts:", debtorsWithLargestDebts);
        ...
        var debtorsWW2 = debtors.Where(d => d.BirthDay.Year <= 1945);
        PrintDebtors("Debtors who witnessed World War II:", debtorsWW2);

        // 17) показать список должников, от имени и фамилии которых можно написать слово «smile».
        var debtorsWithSmileInName = debtors.Where(d => d.FullName.ToLower().Contains("smile"));
        PrintDebtors("Debtors whose name and surname can spell 'smile':", debtorsWithSmileInName);
    }

    static void PrintDebtors(string message, IEnumerable<Debtor> debtors)
    {
